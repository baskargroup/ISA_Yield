...
def process_subfolder(subfolder_path, dest_subfolder_path, ref_dims=None, is_reference=False, used_dates_dict=None):
    ...
    for file in files:
        if is_reference:
            ...
            data = np.load(file)
            T, C, H, W = data.shape
            # Aggregate monthly before filtering by T
            if used_dates_dict is not None:
                monthly_indices, dates = get_monthly_indices(file, used_dates_dict)
                data = aggregate_monthly(data, monthly_indices)
                T = data.shape[0]
            if T < 6:
                continue
...
if __name__ == "__main__":
    ...
    dst_folder = f'processed_data_monthly'
