...
def get_biweekly_indices(dates):
    """
    Group dates by biweek (April 1 to Sept 30, 2 biweeks per month).
    Returns a list of lists, each containing indices of dates for each biweek.
    Total: 12 biweeks (6 months * 2 biweeks)
    
    Biweek 0: Apr 1-15, Biweek 1: Apr 16-30
    Biweek 2: May 1-15, Biweek 3: May 16-31
    ...
    Biweek 10: Sep 1-15, Biweek 11: Sep 16-30
    """
    ...
    biweekly_indices = []
    for b in range(12):  # 6 months * 2 biweeks
    ...
    for modality, file_results in results_dict.items():
        for filename, info in file_results.items():
            ...
            for biweek_idx, status in enumerate(availability):
                ...
                # Create label: "Apr 1-15" or "Apr 16-30"
                if biweek_in_month == 1:
                    biweek_label = f"{month_names[month][:3]} 1-15"
                else:
                    biweek_label = f"{month_names[month][:3]} 16-30"
...
def plot_availability_heatmap(df, output_path='biweek_availability_heatmap.png', year=None):
    ...
    Journal-quality figure optimized for single/double column layout.
    """
    ...
    # Biweek labels for 12 biweeks
    biweek_labels = []
    month_abbrevs = ['Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep']
    for m in month_abbrevs:
        biweek_labels.append(f'{m} 1-15')
        biweek_labels.append(f'{m} 16-30')
    
    modalities = sorted(df['Modality'].unique())
    
    # Single column width is enough for 12 biweeks
    fig_width = DOUBLE_COLUMN_WIDTH
    fig_height = 2.5 + 0.35 * len(modalities)
    ...
    for idx, (status, status_label) in enumerate(zip(status_order, status_labels)):
        ...
        if idx == 2:  # Only show x-labels on bottom plot
            ax.set_xticklabels(biweek_labels, rotation=45, ha='right', fontsize=7)
...
def plot_availability_stacked_bar(df, output_path='biweek_availability_stacked.png', year=None):
    ...
    fig_width = DOUBLE_COLUMN_WIDTH
    fig_height = 1.2 * len(modalities) + 0.5
...
def main():
    parser = argparse.ArgumentParser(description='Analyze biweekly availability for S1GRD, S2L2A, and WEATHER')
    ...
    parser.add_argument('--output-dir', type=str, default='./biweekly_availability_analysis',
    ...
    # Print summary
    print("\n" + "="*80)
    print("SUMMARY STATISTICS (BIWEEKLY)")
    print("="*80)
