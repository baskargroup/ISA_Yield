#!/bin/bash
#SBATCH --time=24:00:00
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=128G
#SBATCH --gres=gpu:a100:1
#SBATCH --partition=nova
#SBATCH --account=mech-ai
#SBATCH --job-name="Test_M1_Corn"
#SBATCH --mail-user=bgekim@iastate.edu
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output="Test_M1_Corn%j.out"
#SBATCH --error="Test_M1_Corn%j.err"
# conda environment
source /work/mech-ai-scratch/bgekim/miniconda3/etc/profile.d/conda.sh
conda activate isa_yield_env
# Test native corn model (24 weeks)
CKPT_FILE="output/corn/M1/differentnorm/lr_e4/best-epoch=094.ckpt"


if [[ -n "$CKPT_FILE" ]]; then
    echo "Testing native corn with checkpoint: $CKPT_FILE"
    terratorch test -c conf_M1/s12d_24_corn.yaml --ckpt_path "$CKPT_FILE"
else
    echo "Error: No checkpoint found for native corn!"
fi
echo "Test finished"