...
class LossHandler:
    ...
    def _compute_loss(self, y_hat: Tensor, ground_truth: Tensor, criterion: Callable):
        # Create a mask where ground truth is non-zero (shape matches ground_truth)
        mask = (ground_truth != 0).float()  # Convert to float for multiplication (0s and 1s)

        # If no non-zero values exist, return zero loss to avoid invalid loss computation
        if mask.sum() == 0:
            return torch.tensor(0.0, device=y_hat.device, dtype=y_hat.dtype, requires_grad=True)

        # Ensure mask has compatible shape for y_hat (e.g., add channel dimension if needed)
        if y_hat.dim() > ground_truth.dim():
            mask = mask.unsqueeze(1)  # Add channel dimension for broadcasting, e.g., [batch, 1, height, width]

        # Apply mask to y_hat: zero out predictions where ground truth is zero
        y_hat = y_hat * mask

        loss: Tensor = criterion(y_hat, ground_truth)
        # If the criterion doesn't natively ignore zeros, scale the loss by the proportion of non-zero values
        # to maintain consistent loss magnitude (optional, depending on criterion)
        non_zero_ratio = mask.sum() / mask.numel()
        if non_zero_ratio > 0:
            loss = loss / non_zero_ratio
        return loss
