...
#SBATCH --job-name="M3_test_lr4"
#SBATCH --mail-user=aapowadi@iastate.edu   # email address
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output="M3_test_lr4%j.out" # job standard output file (%j replaced by job id)
#SBATCH --error="M3_test_lr4%j.err" # job standard error file (%j replaced by job id)
...
echo "Job is starting on `hostname` for M3_test_lr4"

for yaml_file in conf_M3_lre4/*_test.yaml; do
    ...
    ckpt_file=$(find "output/$crop/M3/differentnorm/${modality}_le_4/checkpoints" -name "best-*.ckpt" 2>/dev/null | head -1)
...
echo "Job finished for M3_test_lr4"