...
def sentinel1_download(logger, start_date, end_date, bbox = [-96.6395, 40.3754, -90.1401, 43.5014], output_dir = "./IA_sentinel1"):

    # Define parameters
    collection = "sentinel-1-rtc"  # Landsat 9 Collection 2 Level 2
    ...
    if not items:
        print("No Sentinel-1 images found for the given criteria.")
        ...
        for item in items:
            ...
            for band_name, asset in signed_item.assets.items():
                ...
                try:
                    ...
                    logger.error(f"Failed to download band: {band_name} for the scene: {scene_id}. Error: {e}")
                    continue
...
if __name__ == "__main__":
    ...
    #Start Time
    print("\n==============================================================================================================\n")
    print(f"Downloading Sentinel - 1 images now at {datetime.now()}")
    print("\n==============================================================================================================\n")
    ...
    for _start_date, _end_date in zip(_start_dates, _end_dates):
        try:
            sentinel1_download(my_logger,_start_date, _end_date)
        except Exception as e:
            my_logger.error(f"Failed to download Sentinel - 1 images for the date range: {_start_date} to {_end_date}. Error: {e}")
            continue
    
    #End Time
    end_time = datetime.now()
    my_logger.info(f"Downloaded Sentinel 1 images for the date range: {_start_date} to {_end_date} in {end_time - start_time} seconds.")
