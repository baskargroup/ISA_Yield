        ...
        for band in bands:
            ...
            if not band_files:
                    ...
                    try:
                        with rasterio.open(file) as src:
                            ...
                            if src.crs.to_string() == "EPSG:32614":
                                ...
                                #Resample to 125m
                                # resample_to_new_resolution(file, f"{output_dir}/{_folder}/{src.crs}_R125_{os.path.basename(file)}", src.crs ,125)
                                utm_32614.append(file)
                                
                            elif src.crs.to_string() == "EPSG:32615":
                                
                                #Resample to 125m
                                # resample_to_new_resolution(file, f"{output_dir}/{_folder}/{src.crs}_R125_{os.path.basename(file)}", src.crs ,125)
                                utm_32615.append(file)
                                
                            elif src.crs.to_string() == "EPSG:32616":
                                
                                #Resample to 125m
                                # resample_to_new_resolution(file, f"{output_dir}/{_folder}/{src.crs}_R125_{os.path.basename(file)}", src.crs ,125)
                                utm_32616.append(file)
                                
                            else:
                                print("CRS not supported")
                                continue
                            
                            print(f"Resampling complete! Time taken: {time.time() - resample_time} seconds")
                ...
                try:
                    ...
                    print(f"Merged bands complete! Time taken: {time.time() - merge_time} seconds")
                ...
                try:
                    ...
                    print(f"Reprojection complete! Time taken: {time.time() - reproject_time} seconds")
                ...
                if len(utm_32614) > 0:
                    files_to_merge.append(f"./{output_dir}/{_folder}/32614_WGS84_{band}.tif")
                if len(utm_32615) > 0:
                    files_to_merge.append(f"{output_dir}/{_folder}/32615_WGS84_{band}.tif")
                if len(utm_32616) > 0:
                    files_to_merge.append(f"{output_dir}/{_folder}/32616_WGS84_{band}.tif")
                ...
                print(f"Removed intermediate files for band: {band}. Time taken: {time.time() - remove_time} seconds")
