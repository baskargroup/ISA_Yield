...
def sentinel2_download(logger, start_date, end_date, bbox = [-96.6395, 40.3754, -90.1401, 43.5014], cloud_cover = 10 , output_dir = "./IA_sentinel2"):

    # Define parameters
    collection = "sentinel-2-l2a"  # Landsat 9 Collection 2 Level 2
    bbox =  bbox # Bounding box for Iowa (West, South, East, North)
    date_range = start_date+"/"+end_date  # Example 1-week range
    cloud_cover = cloud_cover  # Maximum cloud cover percentage
...
if __name__ == "__main__":
    ...
    for _start_date, _end_date in zip(_start_dates, _end_dates):
        try:
            sentinel2_download(my_logger,_start_date, _end_date)
        except Exception as e:
            my_logger.error(f"Failed to download Sentinel - 2 images for the date range: {_start_date} to {_end_date}. Error: {e}")
            continue
    
    #End Time
    end_time = datetime.now()
    my_logger.info(f"Downloaded Sentinel 2 images for the date range: {_start_date} to {_end_date} in {end_time - start_time} seconds.")
