...
if __name__ == "__main__":
    ...
    # Set up logger
    logger = setup_logger("./logs/IL_modis_reproject_local.log")

    # Base directory
    base_dir_pattern = "/work/mech-ai-scratch/rtali/gis-modis-IL/IL_modis_NBAR_v2/*"
    base_dirs = glob.glob(base_dir_pattern)

    # Sort the directories
    base_dirs.sort()
    year_list = [str(year) for year in range(2016, 2018)]
    base_dirs = [d for d in base_dirs if d.split('-')[0] in year_list]
    # Output directory
    output_dir = "./reprojected_modis_IL"
    ...
    for folder in tqdm(base_dirs, desc="Processing folders"):
        ...
        for band in bands:
                ...
                for file in band_files:
                    ...
                    try:
                        ...
                        with rasterio.open(file) as src:
                            ...
                            reproject_to_wgs84(
                                file, f"{output_dir}/{_folder}/{os.path.basename(file)}", src.crs)
                        ...
                        with rasterio.open(f"{output_dir}/{_folder}/{os.path.basename(file)}") as src:
                            ...
                            # Define bounding boxes for each UTM zone IL (-91.5131, 36.9701, -87.5247, 42.5083)
                            # zone14_bbox = box(-96.6395, 36.9701, -96, 42.5083)
                            zone15_bbox = box(-91.5131, 36.9701, -90, 42.5083)
                            zone16_bbox = box(-90, 36.9701, -87.5247, 42.5083)

                            # Create GeoDataFrames
                            # gdf_zone14 = gpd.GeoDataFrame(
                            #     {"geometry": [zone14_bbox]}, crs="EPSG:4326")
                            gdf_zone15 = gpd.GeoDataFrame(
                                {"geometry": [zone15_bbox]}, crs="EPSG:4326")
                            gdf_zone16 = gpd.GeoDataFrame(
                                {"geometry": [zone16_bbox]}, crs="EPSG:4326")

                            # Clip into two parts
                            # clip_raster(f"{output_dir}/{_folder}/{os.path.basename(file)}", gdf_zone14,
                            #             f"{output_dir}/{_folder}/CUT_32614_{os.path.basename(file)}")
                            clip_raster(f"{output_dir}/{_folder}/{os.path.basename(file)}", gdf_zone15,
                                        f"{output_dir}/{_folder}/CUT_32615_{os.path.basename(file)}")
                            clip_raster(f"{output_dir}/{_folder}/{os.path.basename(file)}", gdf_zone16,
                                        f"{output_dir}/{_folder}/CUT_32616_{os.path.basename(file)}")

                            # Reproject to UTM zones
                            # Reproject to UTM zones
                            # reproject_raster(f"{output_dir}/{_folder}/CUT_32614_{os.path.basename(file)}",
                            #                  f"{output_dir}/{_folder}/32614_{os.path.basename(file)}", "EPSG:32614")
                            reproject_raster(f"{output_dir}/{_folder}/CUT_32615_{os.path.basename(file)}",
                                             f"{output_dir}/{_folder}/32615_{os.path.basename(file)}", "EPSG:32615")
                            reproject_raster(f"{output_dir}/{_folder}/CUT_32616_{os.path.basename(file)}",
                                             f"{output_dir}/{_folder}/32616_{os.path.basename(file)}", "EPSG:32616")

                            # Resample to 125m
                            # resample_to_new_resolution(f"{output_dir}/{_folder}/32614_{os.path.basename(file)}",
                            #                            f"{output_dir}/{_folder}/R125_32614_{os.path.basename(file)}", "EPSG:32614")
                            resample_to_new_resolution(f"{output_dir}/{_folder}/32615_{os.path.basename(file)}",
                                                       f"{output_dir}/{_folder}/R125_32615_{os.path.basename(file)}", "EPSG:32615")
                            resample_to_new_resolution(f"{output_dir}/{_folder}/32616_{os.path.basename(file)}",
                                                       f"{output_dir}/{_folder}/R125_32616_{os.path.basename(file)}", "EPSG:32616")

                            # Reproject to WGS84
                            # reproject_to_wgs84(f"{output_dir}/{_folder}/R125_32614_{os.path.basename(file)}",
                            #                    f"{output_dir}/{_folder}/WGS84_32614_{os.path.basename(file)}", "EPSG:32614")
                            reproject_to_wgs84(f"{output_dir}/{_folder}/R125_32615_{os.path.basename(file)}",
                                               f"{output_dir}/{_folder}/WGS84_32615_{os.path.basename(file)}", "EPSG:32615")
                            reproject_to_wgs84(f"{output_dir}/{_folder}/R125_32616_{os.path.basename(file)}",
                                               f"{output_dir}/{_folder}/WGS84_32616_{os.path.basename(file)}", "EPSG:32616")

                            # Merge the reprojected files into one
                            wgs_files = [
                                # f"{output_dir}/{_folder}/WGS84_32614_{os.path.basename(file)}",
                                f"{output_dir}/{_folder}/WGS84_32615_{os.path.basename(file)}",
                                f"{output_dir}/{_folder}/WGS84_32616_{os.path.basename(file)}"]

                            # Create a folder for the band
                            os.makedirs(
                                f"./final_modis_data_IL/{_folder}", exist_ok=True)
                            write_file_name = os.path.basename(
                                file).replace(".tif", "").strip()
                            merge_files(
                                wgs_files, f"4326_{write_file_name}", band, f"./final_modis_data_IL/{_folder}")
            ...
            print(
                f"Processing time for band {band}: {time.time() - band_processing_time}")
