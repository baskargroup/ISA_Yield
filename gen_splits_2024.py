...
def split_and_save(common_files, output_dir):
    ...
    corn_files = [s for s in files_2024 if 'corn' in s.lower()]
    soybean_files = [s for s in files_2024 if 'soybean' in s.lower()]
    ...
    save_list(files_2024, 'all_2024.txt')
    save_list(corn_files, 'corn_2024.txt')
    save_list(soybean_files, 'soybean_2024.txt')
    print(f"{output_dir}: Files saved: all_2024.txt ({len(files_2024)} files)")
    print(f"  Corn files: {len(corn_files)}")
    print(f"  Soybean files: {len(soybean_files)}")
...
if __name__ == "__main__":
    ...
    base_dir = '.'
    biweekly_folders = [f for f in os.listdir(base_dir) if f.endswith('2024') and os.path.isdir(os.path.join(base_dir, f))]
