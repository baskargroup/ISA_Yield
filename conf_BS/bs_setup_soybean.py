...
# # START_WEEKS = list(range(1, 21))

# # # ✅ modify only here!
# # # ----- corn -----
# # crop_tag = 'corn' 
# # REMOVED_WEEKS = [8, 19]  # corn
# # MODALITIES = ['S2L2A', 'S1GRD', 'CDL', 'DEM', 'WEATHER'] # corn

# # # # ----- soybean -----
# # # crop_tag = 'soybean'  
# # # REMOVED_WEEKS = [17, 14, 5]  
...
# #                 arrays.append(np.load(fpath, mmap_mode='r'))  # ✅ memmap
# #         if arrays:
# #             merged = np.concatenate(arrays, axis=0)
# #             np.save(f"{combo_name}/{mod}/{fname}", merged)

# # ref_txt_dir = f"{WEEKLY_BASE}_{remaining[0]}"
# # for txt in [f'train_{crop_tag}.txt', f'val_{crop_tag}.txt', f'test_{crop_tag}.txt']:
...
#                 fpath = f"{WEEKLY_BASE}_{w}/{mod}/{fname}"
#                 if os.path.exists(fpath):
#                     arrays.append(np.load(fpath))  # ✅ mmap_mode 제거
#             if arrays:
#                 merged = np.concatenate(arrays, axis=0)
#                 np.save(f"{combo_name}/{mod}/{fname}", merged)
