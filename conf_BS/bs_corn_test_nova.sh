...
#SBATCH --job-name="BS_Corn_Test_R1"
#SBATCH --mail-user=bgekim@iastate.edu
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output="logs/bs_corn/round1/BS_test_round1_%j.out"
#SBATCH --error="logs/bs_corn/round1/BS_test_round1_%j.err"

# ========================================================
# ✅ Please edit only this section for each round!
# ========================================================
round=1
REMOVED_WEEKS=()  # previously removed weeks
# ========================================================
...
mkdir -p predictions/bs_corn/w20/round${round}
mkdir -p plots/bs_corn/w20/round${round}
...
for rm_week in $(seq 1 20); do
    ...
    if [ ${#REMOVED_WEEKS[@]} -eq 0 ]; then
        combo="${rm_week}"
        YAML_FILE="w20_yaml/bs_yamls_corn_bs20_round${round}/config_bs_corn_rm_${rm_week}.yaml"
        CKPT_DIR="output/corn/BS_S12WDC/bs20/round${round}/rm_${rm_week}/checkpoints"
    else
        combo="${removed_str}_${rm_week}"
        YAML_FILE="w20_yaml/bs_yamls_corn_bs20_round${round}/config_bs_corn_rm_${removed_str}_${rm_week}.yaml"
        CKPT_DIR="output/corn/BS_S12WDC/bs20/round${round}/rm_${removed_str}_${rm_week}/checkpoints"
    fi

    out_csv="predictions/bs_corn/w20/round${round}/S12wdc_Corn_bs20_round${round}_rm_${combo}.csv"
    ...
    if [ -n "$OLD_CSV" ]; then
        mv "$OLD_CSV" "predictions/bs_corn/w20/round${round}/S12wdc_Corn_bs20_round${round}_rm_${combo}.csv"
        echo "📄 Saved: ${out_csv}"
    else
    ...
    if [ -n "$OLD_PLOT" ]; then
        mv "$OLD_PLOT" "plots/bs_corn/w20/round${round}/S12wdc_Corn_bs20_round${round}_rm_${combo}.png"
    fi
