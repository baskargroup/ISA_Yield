...
def process_subfolder(subfolder_path, dest_subfolder_path, ref_dims=None, is_reference=False, ts=1):
    if is_reference:
    ...
    for file in rectangular_files:
        if is_reference:
            ...
            T, C, H, W = data.shape
            if T > 4:
                indices = np.round(np.linspace(0, T - 1, ts)).astype(int)
                data = data[indices]
...
def process_root_folder(root_path, dest_root_path, ts=1):
    ...
    for subfolder in sorted(os.listdir(root_path)):
        ...
        if os.path.isdir(subfolder_path):
            if subfolder == 'yield_geotiffs':
                ...
                process_subfolder(subfolder_path, dest_subfolder_path, is_reference=True, ts=ts)
            else:
                print(f"Processing folder: {subfolder}")
                process_subfolder(subfolder_path, dest_subfolder_path, ref_dims=ref_dims, ts=ts)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        try:
            ...
            print("Invalid argument for ts, using default ts=1")
            ts = 1
    else:
        ts = 1
    dst_folder = f'processed_data_{ts}'
    os.makedirs(dst_folder, exist_ok=True)
    process_root_folder('./unprocessed_data', dst_folder, ts=ts)